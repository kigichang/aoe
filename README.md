...
共 1145 則事件，時間範圍西元前 3000 年至
西元 2026 年（見 `src/topics/world/timeline.yaml`）。全部事件皆附維基百科出處（1797 條連結，中/日/英三個站台，均已透過維基 API 驗證條目存在且非消歧義頁）。
...
- **台灣經濟發展史**（`/tw-economy/`）：欄位是**產業**——農業、工業、科技業。
  155 則事件，1624 至 2026 年，附 233 條出處連結：225 條維基百科（中／英兩個
...
- **台灣政治與政黨發展史**（`/tw-politics/`）：欄位是**地區**——台灣、中國。
  219 則事件，1858 至 2026 年，附 299 條出處連結：298 條維基百科（中文站台，
  去重後 267 個條目均已透過 API 驗證存在且非消歧義頁），另有 1 條指向全國法規
