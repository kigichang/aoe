...
根網址（`/`）是**台灣史**：以台灣為主體，把台灣、日本、中國，以及台日中以外的
世界放在同一條軸上，1500 至 2026 年，共 683 則事件（見 `src/topics/taiwan/`）。
資料是從世界史主題複製 1500 年以後的部分起頭的，之後兩份獨立維護 ——
台灣欄會收世界史嫌太細的事件，起點 1500 的取捨寫在 `timeline.yaml`。

**世界史**（`/world/`）是這個站最早的主題：收錄台灣史、日本史、中國史，以及台日中以外的
世界史（歐洲、美洲、中東、南亞）共 1153 則事件，時間範圍西元前 3000 年至
西元 2026 年（見 `src/topics/world/timeline.yaml`）。全部事件皆附維基百科出處（1815 條連結，中/日/英三個站台，均已透過維基 API 驗證條目存在且非消歧義頁）。

站上另外還有六個主題。**多數主題的欄位都不是地區**——這個站的並排比較
不限於地理，換一個維度分欄就換一組問得出來的問題：

- **台灣鐵道史**（`/tw-railway/`）：欄位是**營運者**。134 則事件，1874 至 2026 年，
  附 182 條維基百科出處（同樣驗證過）。台鐵、高鐵捷運、糖鐵三欄已走完完整查證
  流程，林鐵礦鐵一欄仍是種子資料。
...
- **世界藝術史**（`/art/`）：欄位是**門類**——造形、建築、音樂、文學與表演。
  ...
  歌劇剛剛被發明出來」。
- **台灣經濟發展史**（`/tw-economy/`）：欄位是**產業**——農業、礦業、工業、科技業。
  187 則事件，1624 至 2026 年，附 293 條出處連結：285 條維基百科（中／日／英三個
  站台，均已透過 API 驗證條目存在且非消歧義頁），另有 8 條指向農業部、農業
  主管法規、全國法規資料庫、立法院法制局與行政院的官方連結，補維基沒有條目
  的那幾則（原料採取區域制、加速農村建設重要措施、稻穀保價收購、稻田轉作、
  農村再生條例、兩兆雙星）。問得出的是「工研院成立那一年，肥料換穀剛剛廢止」
  「加工出口區設立時，農村還在拿稻穀換肥料」——也就是**台灣如何從農業社會
  長成半導體代工中心**，而那條路徑只有把四個產業並排才看得出來。
  科技業欄在 1972 年以前完全空白，礦業欄則是 1876 年以前與 2000 年以後空白，
  那都是史實不是缺資料——礦業是站上唯一一個有完整結局的產業。

- **台灣政治與政黨發展史**（`/tw-politics/`）：欄位是**地區**——台灣、中國。
  222 則事件，1858 至 2026 年，附 305 條出處連結：304 條維基百科（中文站台，
  去重後 271 個條目均已透過 API 驗證存在且非消歧義頁），另有 1 條指向全國法規
  資料庫，補維基沒有條目的《國家安全法》。這是除了世界史與台灣史之外唯一照地區分欄的主題，
  因為這裡要問的本來就是地理性的：**同一個年代，海峽兩邊各自在走哪一條路。**
  問得出的是「台灣文化協會成立那年，中國正在辦五四之後第一波政黨重組」
  「台灣首次總統直選那年，對岸正在台海試射飛彈」——而最關鍵的一組在 1949：
  **中華民國從中國欄搬到台灣欄，兩欄的斷代色帶在同一條線上一起換色。**
  1858–1894 台灣欄只有七則、遠少於中國欄，那是史實不是缺資料——
  台灣在建省之前沒有政黨、沒有選舉、沒有自主的政治機構。
...
一個**主題**是一份獨立的資料集加上一個網址。台灣史掛在根網址，其餘掛在
`/<目錄名>/`：
...
root: true                       # 掛在根網址。**恰好一個主題可以設**（目前是台灣史）
```
