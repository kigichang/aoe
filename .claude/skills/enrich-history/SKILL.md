...
1. 從使用者的指示判斷目標地區/欄位（`src/topics/<主題>/<地區id>/events.yaml`）。
   世界史主題（`src/topics/world/`）目前有 `taiwan`／`japan`／`china`／`global`
   四欄，前三欄的 `id` 前綴是 `tw-`／`jp-`／`cn-`。
   **`global` 是例外：它涵蓋台日中以外的全世界，一欄內用子區前綴分流** ——
   `eu-` 歐洲、`am-` 美洲、`me-` 中東、`sa-` 南亞（`eu-` 是改名前留下的，
   刻意不動，改了會讓既有的 `#e=` 分享連結失效）。
   前綴與資料夾名別靠記憶硬編，開工前先讀 `regions.yaml` 確認。
